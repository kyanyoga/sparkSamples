...
object StructuredStreaming {
   ...
   def parseDateField(field: String): Option[String] = {
      ...
      if (dateMatcher.find) {
              val dateString = dateMatcher.group(1)  // get multiple fields per line
   ...
   def main(args: Array[String]) {
      ...
      val spark = SparkSession
        ...
        .config("spark.sql.streaming.checkpointLocation", "/Users/kyanyoga/bin/cp")
        .getOrCreate()
        
      setupLogging()
        
      // Create a stream of text files dumped into the logs directory // logs off main or could be s3 || HDFS
