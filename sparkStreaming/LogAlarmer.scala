...
object LogAlarmer {
  ...
  def main(args: Array[String]) {
    ...
    // Extract the status field from each log line : field 6 status code
    ...
    // Now map these status results to success and failure: add up status and errors
    val successFailure = statuses.map(x => {
      val statusCode = util.Try(x.toInt) getOrElse 0  // result of x to int or you get 0: needed for if.
