...
object LogSQL {
  ...
  def main(args: Array[String]) {
    ...
    val requests = lines.map(x => {
      ...
      if (matcher.matches()) {
        val request = matcher.group(5)  // 5th field from regex parser
        val requestFields = request.toString().split(" ")
        val url = util.Try(requestFields(1)) getOrElse "[error]"
        (url, matcher.group(6).toInt, matcher.group(9)) // all fields we want from one line
      } else {
        ("error", 0, "error")
      }
    })
